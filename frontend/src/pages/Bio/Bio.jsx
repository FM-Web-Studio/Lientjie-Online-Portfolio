...
import { getBioProfile, getBioSection } from '../../lib/firestore'
import Skeleton, { TimelineSkeleton, SkillSkeleton } from '../../components/UI/Skeleton'
...
export default function Bio() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([p, edu, exp, sk]) => {
        setProfile(p); setEducation(edu); setExperience(exp); setSkills(sk)
      })
      .catch(console.error)
      ...
      <section className={styles.hero}>
        <div className="container">
          ...
          {loading ? (
            ...
            <>
              <h1 className={styles.name}>{profile?.name ?? 'Lientjie Meiring'}</h1>
              ...
              <div className={styles.profileText}>
                ...
                {loading ? (
                  <div className={styles.skelStack}>
                    {[90, 85, 70, 55].map(w => <Skeleton key={w} width={`${w}%`} style={{ display: 'block', marginBottom: 8 }} />)}
                  </div>
                ) : (
                ...
                {!loading && profile && (
                  <div className={styles.contacts}>
                    {profile.email && <a href={`mailto:${profile.email}`} className={styles.contactLink}>{profile.email}</a>}
                    {profile.phone && <a href={`tel:${profile.phone}`}    className={styles.contactLink}>{profile.phone}</a>}
                    {profile.location && <span className={styles.contactText}>{profile.location}</span>}
                  </div>
                )}
              </div>
            </Reveal>
